...
- **パスキーは 2 つ登録する**(1 アカウントに複数登録できる)
  - iCloud キーチェーン: Mac 2台 + iPad をカバー(1 つのパスキーが同期される)
  - Google パスワードマネージャー: Xiaomi 13T 用
  - 2 エコシステムに分散するので、片方を失ってもログイン手段が残る。全滅時は DB を直接触って再ブートストラップ
- 認可サーバーに「ログイン済み状態で新デバイスのパスキーを追加」できる極小の管理ページを 1 枚持たせる。Xiaomi の初回だけ QR のクロスデバイス認証で入り、その場で本体のパスキーを登録
